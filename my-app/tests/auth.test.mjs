...
test("login, encrypted session, role checks, origin checks, logout and tampering", async () => {
  ...
  const login = load("../app/api/auth/login/route.ts", { "next/server": next, "@/lib/db": { prisma }, "@/lib/auth": auth, "@/lib/passwords.mjs": { verifyPassword }, "@/lib/login-limit": { allowLogin: () => true } });
