...
import { credentialVersion, getSession, isManagementUser, validOrigin } from "@/lib/auth";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!user || !verified || !isManagementUser(user)) return NextResponse.json({ error: "Invalid email or password." }, { status: 401 });
