...
export default function OrderForm({ onClose, onComplete }: { onClose: () => void; onComplete: (order: AllocatedOrder) => void }) {
  ...
  return (
    <ManagementDialog title="Create Order" busy={busy} onClose={onClose}>
      <form onSubmit={submit} noValidate aria-busy={busy}>
        <div className="space-y-6 px-6 py-6">
          <fieldset disabled={busy} className="space-y-4"><legend className="mb-3 text-sm font-semibold">Customer Location</legend>
          ...
          <fieldset disabled={busy || loading || !!catalogError} className="min-w-0 space-y-5"><legend className="mb-3 text-sm font-semibold">Order Items</legend>
