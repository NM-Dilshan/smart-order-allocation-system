...
import { Building2, Package, Plus, RefreshCw } from "lucide-react";
...
export default function OrderManager() {
  ...
  const load = useCallback(() => {
    ...
    return api<Order[]>("/api/orders", { signal: controller.signal }).then((orders) => {
    ...
    <div className="min-h-screen bg-zinc-50 font-sans text-zinc-900 [color-scheme:light]">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-8 sm:py-12">
        ...
        <div className="overflow-x-auto" role="region" aria-label="Orders table" tabIndex={0}><table className="w-full min-w-[900px] table-fixed text-left text-sm" aria-busy={loading}>
          ...
          <tbody className="divide-y divide-zinc-200 bg-white">{loading || error || !orders.length ? <tr><td colSpan={6} className="px-4 py-16 text-center text-zinc-500"><Package size={28} className="mx-auto mb-3 text-teal-700" aria-hidden="true" /><span role="status">{loading ? "Loading orders..." : error ? "Orders unavailable." : "No orders yet."}</span></td></tr> : orders.map((order) => <tr key={order.id} className="align-top hover:bg-zinc-50"><th scope="row" className="px-4 py-5 font-mono font-normal">#{order.id}</th><td className="break-words px-4 py-5 text-xs font-medium text-teal-800">{order.status}</td><td className="px-4 py-5"><ul className="space-y-2">{order.items.map((item) => <li key={item.id} className="break-words [overflow-wrap:anywhere]">{item.product.name} <span className="text-zinc-500">x {item.quantity} · {item.product.price}</span></li>)}</ul></td><td className="break-all px-4 py-5 tabular-nums">{order.customerLatitude}<br />{order.customerLongitude}</td><td className="break-words px-4 py-5 [overflow-wrap:anywhere]">{order.branch?.name ?? "Pending Allocation"}</td><td className="px-4 py-5 text-xs text-zinc-600"><time dateTime={order.createdAt}>{new Date(order.createdAt).toLocaleString()}</time></td></tr>)}</tbody>
        </table></div>
      </main>
